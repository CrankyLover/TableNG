...
from sklearn.metrics import accuracy_score, f1_score, r2_score, mean_squared_error, log_loss, precision_score, recall_score, confusion_matrix
...
class SantosMLEvaluator:
    def __init__(self, query_table_name, true_results_path="../stats/santos_benchmark_true_result_by_santos_full.csv"):
        """初始化评估器"""
        ...
        # 加载相似表格列表
        self.similar_tables = self._load_similar_tables()
        print(f"找到 {len(self.similar_tables)} 个相似表格")
...
def main():
    ...
    evaluator = SantosMLEvaluator(args.query_table)
