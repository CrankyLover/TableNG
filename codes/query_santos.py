...
import sys
import codes.generalFunctions as genFunc
import codes.expandSearch as expand
    ...
    for i in range(0, total_cols - 1):
        if genFunc.getColumnType(inputTable.iloc[:, i].tolist()) == 1:  # the subject in rdf triple should be a text column
            for j in range(i + 1, total_cols):
                if genFunc.getColumnType(
                        inputTable.iloc[:, j].tolist()) == 1:  # the subject in rdf triple should be a text column
                    mergeRelSem = {}
                                            ...
                                            if relation_forward != "None":
                                                ...
                                                for s in relation_forward:
                                                    if s in semantic_dict_forward:
                                                        semantic_dict_forward[
                                                            s] += 1  # relation semantics in forward direction
                                                    else:
                                            ...
                                            if relation_backward != "None":
                                                ...
                                                for s in relation_backward:
                                                    if s in semantic_dict_backward:
                                                        semantic_dict_backward[
                                                            s] += 1  # relation semantics in reverse direction
                                                    else:
                                                        semantic_dict_backward[s] = 1
                if len(semantic_dict_forward) > 0:
                    relation_bag_of_words.append((max(semantic_dict_forward, key=semantic_dict_forward.get) + "-r",
                                                  str(i) + "_" + str(j),
                                                  max(semantic_dict_forward.values()) / total_kb_forward_hits))
                    relation_dependencies.append(str(i) + "-" + str(j))
                    relation_dictionary[str(i) + "-" + str(j)] = [(max(semantic_dict_forward, key=semantic_dict_forward.get),
                                                                   max(semantic_dict_forward.values()) / total_kb_forward_hits)]
                    total_hits[str(tab_id) + "-" + str(i) + "-" + str(j)] = (total_kb_forward_hits, unique_rows_in_pair)
                if len(semantic_dict_backward) > 0:
                    relation_bag_of_words.append((max(semantic_dict_backward, key=semantic_dict_backward.get) + "-r",
                                                  str(j) + "_" + str(i),
                                                  max(semantic_dict_backward.values()) / total_kb_backward_hits))
                    relation_dependencies.append(str(j) + "-" + str(i))
                    relation_dictionary[str(j) + "-" + str(i)] = [(max(semantic_dict_backward, key=semantic_dict_backward.get),
                                                                   max(semantic_dict_backward.values()) / total_kb_backward_hits)]
        ...
        if genFunc.getColumnType(input_table[columnName].tolist()) == 1:  # check column Type
            ...
            if all_top_types:
                ...
                if scoring_function == 1:
                    column_bag_of_words.append(
                        (selected_top_type + "-" + label, str(col_id), all_found_types[selected_top_type] / total_kb_hits))
...
def main():
    # 前端进程开始的时候就要开始运行这个程序了，但是这个程序这个时候需要做一些预处理的工作，因此用户不能立即开始选表查询
    ...
    # 这个时候预处理完了，给前端发送一个处理好了的信号，然后后端程序阻塞在这里
    # 阻塞
    # 接收到前端发送来的 query_table_file_name， 开始运行下面的程序段（可以做循环也可以不做，取决于是否要用户查多次，还是一次就行）
    ...
    for table in query_table_file_name:
        ...
        if which_mode == 2 or which_mode == 3:
            synthetic_relation_dictionary, synthetic_triples_dictionary, synth_subject_semantics = computeSynthRelation(
                input_table, subject_index, synth_relation_kb)
        ...
        for key in sortedTableList:
            data_lake_table = key[0].split(os.sep)[-1]
            return_list.append(data_lake_table)
            if enum < 5:
        ...
        # 这个地方查询结束，给前端返回去return_list（表名称的列表），dtp，dfp，totalPositive - dtp（TP，FP，FN三个查询精度的指标）
